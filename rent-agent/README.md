...
Если с VPS нет прямого доступа к `api.telegram.org` (ошибка `ETIMEDOUT 149.154.x.x`), **исходящие** вызовы Bot API (`sendMessage`, `answerCallbackQuery`, `getUpdates` в режиме polling) нужно пустить через локальный Xray:
...
| Telegram | `TELEGRAM_BOT_TOKEN`, `TELEGRAM_OWNER_CHAT_ID`, `TELEGRAM_MODE`, `TELEGRAM_PROXY` (опц.) | @BotFather; chat_id — через `/start` и логи; `TELEGRAM_MODE=webhook` или `polling` — см. ниже |
...
Переменная **`TELEGRAM_MODE`**: `webhook` (по умолчанию) или `polling`.

| Режим | Когда использовать | Входящий трафик |
|-------|-------------------|-----------------|
| `webhook` | VPS с открытым HTTPS, Telegram достучится до сервера | Telegram → `POST /webhook/telegram` |
| `polling` | RU-VPS, `Connection timed out` в `getWebhookInfo` | Сервер → `getUpdates` (исходящий) |

Webhook и polling **нельзя** включать одновременно. В режиме `polling` агент при старте сам вызывает `deleteWebhook`.

### Режим webhook (по умолчанию)
...
# url = ${PUBLIC_URL}/webhook/telegram, без last_error_message
```
...
### Режим polling (RU-VPS / firewall)

Если Telegram не может достучаться до вашего сервера (`pending_update_count` растёт, `Connection timed out`), переключитесь на polling:

1. В `.env` на сервере:

```env
TELEGRAM_MODE=polling
# при блокировке api.telegram.org — также TELEGRAM_PROXY (см. выше)
```

2. Снимите webhook (агент делает это сам при старте; можно вручную):

```bash
curl -s "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/deleteWebhook"
```

3. Перезапустите агента (`npm start` / pm2 restart).

В логах:

```
TELEGRAM_MODE=polling
[telegram] Webhook снят (режим polling)
[telegram] Polling запущен
```

4. Напишите боту `/start` — в логах: `[webhook/telegram] chat_id=...` (тот же обработчик, другой транспорт).

> **Важно:** должен работать **один** процесс агента на токен — два poller'а конфликтуют.  
> E2E-тесты (`e2e_mock.js`) по-прежнему используют `POST /webhook/telegram` in-process — для них `TELEGRAM_MODE` не меняйте.
...
1. `TELEGRAM_MODE=webhook` → `setWebhook` для Telegram → `${PUBLIC_URL}/webhook/telegram`  
   (или `TELEGRAM_MODE=polling` — webhook не нужен)
...
8. Зарегистрируйте вебхуки Авито и RealtyCalendar на публичный URL.
9. Telegram: `TELEGRAM_MODE=webhook` → `setWebhook`; `TELEGRAM_MODE=polling` → webhook не нужен (см. раздел «Telegram: webhook или polling»).
...
| `POST` | `/webhook/telegram` | `routes/telegram.js` | `200 OK` сразу (только при `TELEGRAM_MODE=webhook`) |
