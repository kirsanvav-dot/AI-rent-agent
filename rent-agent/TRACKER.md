...
## Шаг 12 — Сквозной end-to-end тест (предпоследний) `[done]`
...
## Шаг 13 — README + runbook (последний) `[done]`
...
**Задачи:**
- [x] В `rent-agent/README.md` написать разделы:
  - [x] **Быстрый старт:** `git clone` → `cd rent-agent && npm install` → `cp .env.example .env` → заполнить → `node index.js`
  - [x] **Notion-миграция** (`CHANGE_REQUEST.md` §7.2): чек-лист из 5 шагов с предупреждением «выполнить ДО деплоя»
  - [x] **Регистрация вебхука Telegram:** `curl` к `setWebhook` с `<PUBLIC_URL>/webhook/telegram`
  - [x] **Регистрация вебхука Авито (FR-18, CR-ADD-11):** как прописать URL в кабинете Авито или через API подписки на события Messenger; проверка боевым сообщением
  - [x] **RealtyCalendar (исходящий API):** где взять `REALTYCALENDAR_API_URL/TOKEN/OBJECT_ID`; тест блокировки одной даты
  - [x] **Локальный тест через ngrok**
  - [x] **Деплой:** Railway / Render — переменные окружения через UI платформы (NFR-1, риск R-05)
  - [x] **Эндпоинты:** таблица из `ARCHITECTURE.md` §12
  - [x] **Чек-лист релиза:** из `SPECIFICATION.md` §6.3
  - [x] **Известные ограничения:** `DEMO_MODE`, одна квартира на один `REALTYCALENDAR_OBJECT_ID`, верификация подписей вне скоупа (см. SPEC §6.1)
- [x] Сделать grep-проверку: в README нет хардкоженных токенов / личных chat_id / реальных URL аккаунта
- [x] **Регрессия:**
  - [x] Полный регрессионный набор + `e2e_full_flow.js` зелёный (37/37 + 6/6)
  - [x] Перепрочитать README глазами «нового разработчика»: после клона + следования инструкции — система стартует и здоровый ответ `GET /health` приходит
...
| 12 | Сквозной end-to-end тест | Тестирование | проверяет всё | `[done]` |
| 13 | README + runbook | Документация | CR-ADD-11 | `[done]` |
